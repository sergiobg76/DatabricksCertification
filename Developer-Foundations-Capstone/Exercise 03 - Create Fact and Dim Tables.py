...
data = spark.read.format(read_format).load(batch_source_path)
data.cache().createOrReplaceTempView("batched_orders")
...
data = spark.sql("SELECT * FROM batched_orders")
data.first()
ssn_data = data.withColumn("_error_ssn_format", when(col("sales_rep_ssn").contains("-"), True).otherwise(False))
# sales_rep_ssn to long
formated_data = ssn_data.withColumn("sales_rep_ssn", regexp_replace("sales_rep_ssn", "-", "").cast(LongType())).withColumn("sales_rep_zip", col("sales_rep_zip").cast(IntegerType()))

prep_data  = formated_data.drop("submitted_at","order_id","customer_id","shipping_address_attention","shipping_address_address","shipping_address_city","shipping_address_state","shipping_address_zip","product_id","product_quantity","product_sold_price")

final_data = prep_data.dropDuplicates(["sales_rep_id","sales_rep_ssn","sales_rep_first_name","sales_rep_city","sales_rep_state","sales_rep_zip","_error_ssn_format"])
final_data.createOrReplaceTempView("final_data")
final_data.write.mode("overwrite").saveAsTable("sales_reps")
...
batch_temp_view = spark.sql("select * from batched_orders")
format_data = batch_temp_view.withColumn("submitted_at", from_unixtime(col("submitted_at").cast(IntegerType())).cast(TimestampType())).withColumn("shipping_address_zip", col("shipping_address_zip").cast(IntegerType()).alias("shipping_address_zip"))

select_data = format_data.drop("sales_rep_ssn","sales_rep_first_name","sales_rep_last_name","sales_rep_address","sales_rep_city","sales_rep_state","sales_rep_zip","product_id","product_quantity","product_sold_price")

select_data.dropDuplicates(["submitted_at","order_id","customer_id","sales_rep_id","shipping_address_attention","shipping_address_address","shipping_address_city", "shipping_address_state", "shipping_address_zip"])

prep_data = select_data.withColumn("submitted_yyyy_mm", date_format(col("submitted_at"), "yyyy-MM"))

spark.sql("DROP TABLE IF EXISTS orders")
prep_data.dropDuplicates().write.mode("overwrite").partitionBy("submitted_yyyy_mm").saveAsTable("orders")


# COMMAND ----------

# MAGIC %sql
# MAGIC OPTIMIZE dbacademy_sbolanos_minsait_com_developer_foundations_capstone.orders
...
batch_temp_view = spark.sql("SELECT * FROM batched_orders")
filter_data = batch_temp_view.select("order_id", "product_id", "product_quantity", "product_sold_price", "ingest_file_name", "ingested_at")
prep_data = filter_data.withColumn("product_quantity", col("product_quantity").cast(IntegerType())).withColumn("product_sold_price", col("product_sold_price").cast(DecimalType(10,2)))
prep_data.write.mode("overwrite").saveAsTable("line_items")
